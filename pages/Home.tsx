    ...
    <div className="animate-in fade-in duration-700">
      ...
      <section className="relative h-screen min-h-[700px] flex items-center overflow-hidden">
        <div className="absolute inset-0 z-0">
          ...
          <div className="absolute inset-0 bg-gradient-to-r from-slate-950/90 via-slate-900/60 to-transparent rtl:bg-gradient-to-l"></div>
        </div>
        
        <div className="container mx-auto px-6 relative z-10">
          <div className="max-w-4xl space-y-6">
            <div className="inline-flex items-center space-x-2 rtl:space-x-reverse bg-red-600 px-4 py-1.5 rounded-full text-white text-xs font-black uppercase tracking-widest animate-bounce">
            ...
            <p className="text-xl md:text-2xl text-slate-200 font-medium max-w-xl">
              {isEn 
            ...
            <div className="flex flex-wrap gap-4 pt-8">
              <button 
                ...
                className="group relative px-12 py-5 bg-red-600 text-white rounded-xl text-xl font-black overflow-hidden shadow-[0_10px_40px_-10px_rgba(220,38,38,0.5)] transition-all hover:-translate-y-1 active:scale-95"
              >
              ...
              <button 
                ...
                className="px-12 py-5 bg-white/10 backdrop-blur-md text-white border border-white/20 rounded-xl text-xl font-black hover:bg-white/20 transition-all"
              >
      ...
      <section className="py-32 bg-[#001F2D] text-white">
        <div className="container mx-auto px-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[
              ...
              <div 
                key={idx} 
                className="group flex flex-col items-center text-center p-10 bg-white/5 border border-white/10 rounded-[3rem] hover:bg-white/10 transition-all duration-500 hover:-translate-y-4"
              >
                <div className="mb-8 w-24 h-24 bg-red-600 rounded-[2rem] flex items-center justify-center rotate-3 group-hover:rotate-12 transition-transform shadow-2xl shadow-red-600/20">
                  <i className={`fas ${service.icon} text-4xl text-white`}></i>
                </div>
                <h3 className="text-3xl font-black mb-4">{isEn ? service.titleEn : service.titleAr}</h3>
          ...
          <div className="flex flex-col lg:flex-row items-center gap-16 lg:gap-24">
            <div className="w-full lg:w-1/2 space-y-8">
              <div className="inline-block">
                <h2 className="text-4xl md:text-5xl font-black text-[#001F2D] relative z-10">
                  {isEn ? 'Two Fun Parks In One' : 'منتزهان في مكان واحد'}
                  <div className="absolute -bottom-2 left-0 w-full h-4 bg-red-600/10 -z-10"></div>
                </h2>
              </div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                {[
                  ...
                  <div key={i} className="flex items-center space-x-3 rtl:space-x-reverse bg-slate-50 p-4 rounded-2xl border border-slate-100">
                    <i className={`fas ${item.icon} text-red-600`}></i>
                    <span className="font-bold text-sm text-[#001F2D]">{item.label}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="w-full lg:w-1/2 relative">
              <div className="relative z-10 rounded-[4rem] overflow-hidden rotate-2 shadow-2xl border-[16px] border-white">
                <img 
              ...
              {/* Decorative Elements */}
              <div className="absolute -top-10 -right-10 w-40 h-40 bg-red-600 rounded-full mix-blend-multiply filter blur-3xl opacity-20"></div>
              <div className="absolute -bottom-10 -left-10 w-64 h-64 bg-blue-600 rounded-full mix-blend-multiply filter blur-3xl opacity-20"></div>
              <div className="absolute top-1/2 -left-8 -translate-y-1/2 bg-red-600 text-white p-6 rounded-3xl shadow-2xl z-20 -rotate-12 hidden md:block">
                <div className="text-3xl font-black">2600+</div>
                <div className="text-xs font-bold uppercase tracking-widest">{isEn ? 'Square Meters' : 'متر مربع'}</div>
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="py-24 bg-red-600 relative overflow-hidden">
        <div className="absolute inset-0 opacity-10 pointer-events-none">
           <div className="grid grid-cols-6 gap-4 transform -rotate-12">
             {Array.from({length: 24}).map((_, i) => (
               <i key={i} className="fas fa-bolt text-9xl text-white"></i>
             ))}
           </div>
        </div>
        <div className="container mx-auto px-6 text-center relative z-10">
           ...
           <button 
             onClick={() => onNavigate(Page.Contact)}
             className="px-16 py-6 bg-white text-red-600 rounded-2xl text-2xl font-black shadow-2xl hover:scale-110 active:scale-95 transition-all"
           >
